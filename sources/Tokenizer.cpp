...
namespace Tokenizer
{
    ...
    {
        std::string curr_token, curr_operator;
        ...
        for(int i=0; i<static_cast<int>(a_command.size()); i++)
        {
            ...
            else if(a_command[i]=='|'  || a_command[i]=='&' || a_command[i]=='>' || a_command[i]=='<' || a_command[i]==';')
            {
                ...
                if((a_command[i]=='|' || a_command[i]=='&' || a_command[i]=='>') && i!=static_cast<int>(a_command.size())-1 && a_command[i]==a_command[i+1])
                {
                    curr_operator+=a_command[i+1];
                    i++;
                }
            ...
            else
            {
                ...
                if(a_command[i]=='\"' || a_command[i]=='\'')
                {
                    int j=i;
                    i++;
                    for(;i<static_cast<int>(a_command.size());i++)
                    {
